...
We report preliminary descriptive statistics from the four respondents that have completed the full DDI panel at the time of writing: two closed-API generative models---GPT-4o (\texttt{gpt-4o-2024-11-20}) and Claude Sonnet 4.5 (\texttt{claude-sonnet-4-5})---and two contrastive zero-shot models---CLIP ViT-L/14 (\texttt{openai/clip-vit-large-patch14}) and BiomedCLIP (\texttt{microsoft/BiomedCLIP-PubMedBERT\_256-vit\_base\_patch16\_224}). A third contrastive respondent (SigLIP-L/16) and the open-weight VLM panel are still being collected; the pre-registered amortized Rasch primary endpoint (\S\ref{sec:aggregate}) is not yet estimated here. We present the conventional fairness-audit framing on the current four-respondent panel as the comparator against which the pre-registered IRT decomposition will later be evaluated, and three descriptive findings from this partial panel that already inform how that decomposition should be read.
...
openai/clip-vit-large-patch14 & 656 & 0.721 (208) & 0.647 (241) & 0.691 (207) & $-$0.030 \\
microsoft/BiomedCLIP-PubMedBERT\_256-vit\_base\_patch16\_224 & 656 & 0.562 (208) & 0.598 (241) & 0.517 (207) & $-$0.046 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../artifacts/prelim/prediction_distribution.pdf}
