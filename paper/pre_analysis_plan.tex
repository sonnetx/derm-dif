...
We ran the pre-registered amortized Rasch fit (\S\ref{sec:amortized}) on the five-respondent panel with the BiomedCLIP-embedding backbone and applied the primary-endpoint decision rule (\S\ref{sec:aggregate}). The result:
\begin{equation*}
    \hat\Delta = +0.593 \text{ logits}, \quad \text{95\% bootstrap CI } [-0.347, +1.594], \quad \text{decision: indeterminate}.
\end{equation*}
The point estimate exceeds the pre-registered $0.5$-logit falsification threshold and is directionally consistent with the raw FST-stratified accuracy gap: V--VI items are harder by $\approx 0.6$ logits after residualizing against lesion category and malignancy. The CI is too wide to exclude null at $J = 5$; this is the expected regime for a small respondent panel, and tightening the CI is what the open-weight VLM expansion is intended to accomplish.

We highlight that the point estimate is \emph{stable} across two consecutive fits with different panel composition: a preliminary $J = 6$ fit that included an all-NaN SigLIP row and a partial Gemini row returned $\hat\Delta = +0.602$ logits; after dropping both via a coverage filter, the clean $J = 5$ fit returns $+0.593$. The garbage rows contribute near-zero signal as expected, and the small movement is consistent with anchor-induced drift rather than substantive change.

The fitted item-difficulty distribution ranges over $[-36, +13]$ logits, well outside the typical Rasch range of $\pm 3$. This artifact reflects $166$ items at the response-pattern floor (all five respondents incorrect, $n = 30$) or ceiling (all correct, $n = 136$): for these items the masked likelihood has no gradient pulling $\hat b_i$ toward a finite value and the embedding-amortized MLP diverges. Per the PAP fallback policy (\S\ref{sec:amortized}), we will either prune these saturated items before fitting or add a stronger $L_2$ prior on $\hat b_i$ in the next iteration. The current $\hat\Delta$ point estimate is insensitive to this---residualization is performed on $\hat b_i$ and saturated items contribute small per-FST residual mass because all respondents agree on them---but the bootstrap CI should tighten meaningfully once these items are handled.
...
We record one deviation from \texttt{config/models.yaml} as frozen at \texttt{prereg-v1}. The originally pre-registered Gemini respondent, \texttt{gemini-2.0-flash}, was deprecated by Google before our query phase completed and now returns 404. We substituted \texttt{gemini-3.1-flash-lite}---the production-tier Flash-family model in the current Gemini line---as the closest analog. (A briefly attempted intermediate substitution to \texttt{gemini-3.1-pro-preview} was abandoned after that model's $\approx 250$ requests/day quota proved insufficient to complete the panel.) No other deviations.
