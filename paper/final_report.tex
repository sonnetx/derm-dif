...
\textit{Panel size.} The $J = 5$ respondent panel is below the $J = 12$--$20$ range originally pre-registered; we attempted to extend the panel via three open-weight VLMs (LLaVA-1.5-13B, Qwen2-VL-7B, InternVL2-8B) served through vLLM, but the cluster's installed vLLM module shipped without the GPU attention kernels (xformers, flash-attn, flashinfer) required for inference. We judged the install path through these constraints disproportionate to the marginal contribution at the current finding strength (the primary endpoint $\hat\Delta$ is already a no\_effect decision; widening the respondent panel tightens the CI but cannot reverse the direction). Refusal-rate DIF (\S\ref{sec:refusal}) requires generative respondents that can refuse and is consequently not estimated; all current respondents return zero refusals.
