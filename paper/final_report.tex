...
\textbf{Respondents ($J = 7$).} Four closed-API generative models---\texttt{openai/gpt-4o-2024-11-20}, \texttt{anthropic/claude-sonnet-4-5}, \texttt{anthropic/claude-haiku-4-5-20251001}, and \texttt{google/gemini-3.1-flash-lite}---queried via the respective provider SDKs at temperature $0$ with $\max\_tokens = 256$. Three contrastive image-text models---\texttt{openai/clip-vit-large-patch14}, \texttt{google/siglip-large-patch16-384}, and \texttt{microsoft/BiomedCLIP-PubMedBERT\_256-vit\_base\_patch16\_224}---queried zero-shot via argmax over cosine similarity between the image embedding and the candidate text labels \texttt{``a photo of a benign skin lesion''} and \texttt{``a photo of a malignant skin lesion''} (CLIP/SigLIP via the HuggingFace \texttt{transformers} \texttt{AutoModel} interface, BiomedCLIP via \texttt{open\_clip}). An eighth model (\texttt{openai/gpt-4o-mini-2024-07-18}) was queried but refused $95.3\%$ of items and is excluded from the IRT fit by the minimum-coverage threshold. The full prompt template (Section~\ref{sec:query}) is frozen at the \texttt{prereg-v1} git tag in \texttt{config/protocol.yaml}.
...
M1 & + lesion category & $0.358$ & --- \\
M2 & + malignancy & $0.358$ & --- \\
M3 & + image features & $0.364$ & --- \\
M4 & + FST & $0.365$ & $-0.129$ $[-0.538, +0.321]$ \\
\midrule
M\_raw & FST only (no controls) & --- & $+0.169$ $[-0.220, +0.526]$ \\
M\_img & image features only & $0.009$ & --- \\
M\_img\_fst & image features + FST & --- & $+0.327$ $[-0.124, +0.755]$ \\
M6 & lesion + malignancy + image (R/G/B) + FST & $0.365$ & $-0.081$ $[-0.504, +0.393]$ \\
\bottomrule
\end{tabular}
\end{center}
Four observations dominate. First, \textbf{lesion category alone explains $36\%$ of the item-difficulty variance} (M1 $R^2 = 0.358$), while all other predictors combined explain less than $0.7\%$ additional variance. Second, \textbf{the raw FST effect is null}: M\_raw, which regresses difficulty on FST with no controls, yields $\beta_{\text{V--VI}} = +0.169$ $[-0.220, +0.526]$---the CI includes zero and the point estimate is nominally in the \emph{easier} direction, opposite the conventional accuracy gap. Third, \textbf{image features do not mediate FST}: M\_img explains only $R^2 = 0.009$ of difficulty variance, and M\_img\_fst shows $\beta_{\text{V--VI}} = +0.327$ $[-0.124, +0.755]$ after image controls---larger in magnitude than M\_raw, ruling out the hypothesis that photography contrast on dark skin drives the FST signal. Fourth---the pre-registered headline---\textbf{adding FST to the full design matrix (M4, M6) produces $\Delta R^2 \approx 0.001$ and FST coefficients indistinguishable from zero}: $\beta_{\text{V--VI}} \in [-0.129, -0.081]$ across specifications, both CIs straddling zero. The sign is nominally negative in the fully-controlled models (V-VI items easier after controls), consistent with the residualized shift $\hat\Delta = -0.031$ from the primary endpoint.

Taken together, the LLTM sensitivity models sharpen the interpretation. The V--VI accuracy gap that appears in conventional stratified evaluation (Table~\ref{tab:prelim-fst-accuracy}) is not reflected as a raw item-difficulty shift in IRT: V-VI items are not harder to begin with, even without any lesion-category control. Lesion composition is the entire story.

\paragraph{Stability under randomization and regularization.}
\label{sec:prelim_seed_lambda}
We ran two robustness sweeps on the regularized fit at the earlier $J = 5$ panel (pre-dating the optional-tier expansion to $J = 7$): a multi-seed sweep over five random seeds controlling MLP initialization, Adam optimization, train/holdout split, and bootstrap resampling; and a sensitivity sweep over the difficulty prior strength $\lambda_b \in \{10^{-3}, 3 \times 10^{-3}, 10^{-2}, 3 \times 10^{-2}, 10^{-1}\}$ spanning two orders of magnitude. The $J = 5$ headline $\hat\Delta$ is stable across seeds (median $+0.125$, range $[+0.120, +0.129]$; all five fits return the same \emph{no\_effect} decision). Across $\lambda_b$: $\hat\Delta$ varies monotonically from $+0.155$ at the smallest prior to $+0.083$ at the largest; every value lies below the $0.5$-logit falsification threshold and every fit returns \emph{no\_effect}. The $J = 7$ primary endpoint ($\hat\Delta = -0.031$) supersedes these $J = 5$ sweeps as the reported finding; the sweeps are included to confirm that the null is not an artifact of a particular seed or prior-strength choice.
...
\midrule
BiomedCLIP, $J=7$ (primary) & $-0.031$ & $[-0.271, +0.275]$ & no\_effect \\
\bottomrule
\end{tabular}
\end{center}

Spearman correlations between the two fits: per-item difficulty $\rho = 0.965$, per-respondent ability $\rho = 1.000$ ($n_{\text{items}} = 656$; this cross-backbone check was run at $J = 5$ prior to the optional-tier expansion). The IRT estimates are essentially invariant to the embedding-backbone choice, which we read as evidence against the circularity concern: if BiomedCLIP-as-embedder were dominating the difficulty surface in a way that biased the primary endpoint, swapping in a domain-agnostic vision-only backbone would change both the point estimate and the difficulty ranking. Neither moved meaningfully.
...
The conventional fairness audit reports a $-5$ to $-7$ percentage-point V--VI vs.\ I--II accuracy gap across the two full-size generative respondents (GPT-4o, Claude Sonnet 4.5; Table~\ref{tab:prelim-fst-accuracy}). The IRT residualization, the LLTM nested decomposition, the saturated-item pruning robustness, the cross-backbone DINOv3 refit, the multi-seed sweep, the $\lambda_b$ prior-strength sweep, and the configural-invariance check are mutually consistent: after partialling out the lesion-category distribution and image-property covariates, the residual FST contribution is statistically indistinguishable from zero in either direction. The point estimate $\hat\Delta = -0.031$ logits with $95\%$ CI $[-0.271, +0.275]$ excludes any V--VI vs.\ I--II item-difficulty shift larger than about $0.28$ logits.

The LLTM sensitivity models (\S\ref{sec:prelim_lltm}) sharpen this further. The raw FST effect---M\_raw, with no lesion or image controls---is already null: $\beta_{\text{V--VI}} = +0.169$ $[-0.220, +0.526]$, a point estimate in the nominally \emph{easier} direction and a CI that fully straddles zero. Image features explain only $R^2 = 0.009$ of item-difficulty variance, and FST after image controls (M\_img\_fst) is $+0.327$ $[-0.124, +0.755]$---ruling out photography contrast on dark skin as the mechanism. The conventional accuracy gap is not present at the IRT item-difficulty level to begin with; the lesion-category composition is the entire story.

We read this as the empirical claim the paper is built to make: \emph{on this benchmark, the FST-stratified accuracy gap that the audit literature reports does not reflect a construct-level FST shift; it reflects the lesion-category composition that differs across FST strata in DDI's curation}. This is a falsifiable methodological claim about what FST-stratified accuracy is measuring; it is not the substantive claim that ``there is no demographic bias in dermatology AI,'' which the IRT null does not establish (\S\ref{sec:limitations}).
